...
description: "Learn how CraftCommand models commands as annotated Java classes with @Default and @Subcommand methods, and how nested static classes enable deep subcommand hierarchies."
---
...
1. If the parameter has `@Suggest("methodName")`, the generator emits a call to that method or field. The referenced name can be either a method (accepting up to three parameters: `sender`, `String[]`, `String`) or a `public` field of type `List<String>`. For example, `@Suggest("getItems")` routes to a method, while `@Suggest("modes")` routes to a `public final List<String> modes` field.
